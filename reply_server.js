...
function seasonCrossSell() {
  ...
  if (ym <= "2026-08") {
    return [
      ...
      "  · 선크림(EGA UV IN&OUT Protector) 리뷰 → NMN 지우개 마스크팩을 추천한다. 자외선 받은 피부 애프터선 진정·회복으로 연결한다.",
...
- ★단, 각 성분은 그 성분이 실제로 든 “구매 제품”을 설명할 때만 쓴다. 예: 세피라 프로텍트(CefiraProtect CLR™)는 마스크팩을 구매한 리뷰에서만 언급하고, NMN 스킨부스터·핸드크림·선크림 리뷰에는 마스크팩 성분(세피라 프로텍트)을 끌어오지 않는다.
