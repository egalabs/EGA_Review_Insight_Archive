...
function seasonCrossSell() {
  ...
  if (ym <= "2026-08") {
    return [
      ...
      "- 아래 우선순위대로 딱 1개만 추천한다 (핸드크림은 반드시 핸드크림 전용 규칙을 먼저 적용):",
      "  · 에센셜 핸드크림 리뷰 → 어드밴스드 핸드크림(야외용, SPF20 자외선 차단)을 추천. 요즘 강한 자외선에 손등이 노출되는 야외 상황과 연결한다.",
      "  · 어드밴스드 핸드크림 리뷰 → 에센셜 핸드크림(실내용, 보습·손 컨디션 유지)을 추천. 실내·사무실에서 수시로 덧바르는 루틴으로 연결한다.",
      "  · 핸드크림 인앤아웃 세트 리뷰 → EGA UV IN&OUT Protector(선크림)를 추천. 손뿐 아니라 얼굴 자외선 케어까지 계절감으로 자연스럽게 연결한다.",
      "  · 리버스 루틴 세트·키트(5일 리버스 키트·30일 리버스 루틴 완성팩·먹는 NMN+핸드크림+마스크팩 구성) 리뷰 → EGA UV IN&OUT Protector(선크림)를 추천. 몸 안팎을 챙기는 토탈케어에 얼굴 데일리 자외선 차단까지 더하는 흐름으로 자연스럽게 연결한다.",
      "  · 선크림(EGA UV IN&OUT Protector) 리뷰 → 마지막 크로스셀은 반드시 NMN 지우개 마스크팩. ‘선크림으로 자외선을 막고(OUT), 그래도 받은 자외선 데미지는 마스크팩으로 회복(애프터선)’ 흐름으로 잇되, 마스크팩의 회복 근거를 ‘세피라 프로텍트(CefiraProtect CLR™)’ 기전 중심으로 짧게 설명한다(자외선·환경 스트레스로 손상된 피부를 보호·회복하는 성분).",
      "  · 그 외 모든 제품(스킨부스터·마스크팩·앰플 등) 리뷰 → EGA UV IN&OUT Protector(선크림)를 추천한다. 자외선 강한 요즘 날씨와 연결한다.",
      "- 구매 제품과 여름 날씨 흐름으로 자연스럽게 이어서, 억지스럽지 않게 1개만 추천한다.",
      "- ★중요: 추천하는 제품은 고객이 아직 구매·사용하지 않은 다른 제품/신제품이다. 따라서 ‘이미 사용 중이신’, ‘꾸준히 쓰시는’, ‘함께 쓰고 계신’처럼 고객이 이미 보유·사용한다는 전제의 표현은 절대 쓰지 않는다. 대신 ‘새로 나온 ~를 한 번 경험해보시길 권해드린다’, ‘~도 함께 사용해보시면 좋다’처럼 새로 소개하고 권유하는 톤으로 쓴다."
  ...
  return [
    ...
    "- 핸드크림 전용 규칙 우선: 에센셜 핸드크림 리뷰 → 어드밴스드 핸드크림 / 어드밴스드 핸드크림 리뷰 → 에센셜 핸드크림 / 핸드크림 인앤아웃 세트 리뷰 → 선크림(EGA UV IN&OUT Protector).",
    "- 리버스 루틴 세트·키트 리뷰 → 토탈케어 흐름에 이어 선크림(EGA UV IN&OUT Protector)을 추천한다.",
    "- 선크림 리뷰 → 마지막 크로스셀은 NMN 지우개 마스크팩. 세피라 프로텍트(CefiraProtect CLR™) 기전 중심으로 자외선 데미지 회복(애프터선) 흐름으로 잇는다.",
    "- 그 외 제품 리뷰 → 건조·보습 니즈와 연결해 핸드크림 또는 NMN 지우개 마스크팩을 추천한다.",
    "- 구매 제품과 자연스럽게 이어서, 억지스럽지 않게 1개만 추천한다.",
    "- ★중요: 추천하는 제품은 고객이 아직 구매·사용하지 않은 다른 제품/신제품이다. ‘이미 사용 중이신’, ‘꾸준히 쓰시는’처럼 이미 보유·사용한다는 전제 표현은 절대 쓰지 않고, ‘한 번 경험해보시길 권해드린다’처럼 새로 소개·권유하는 톤으로 쓴다."
  ].join(String.fromCharCode(10));
}

// ===== 편집 가능한 제품 프롬프트 설정 (prompt_config.json) =====
const PROMPT_CONFIG_PATH = path.join(__dirname, "prompt_config.json");
const DEFAULT_PRODUCT_ORDER = ["essential","advanced","inoutSet","skinbooster","maskpack","ampoule","suncream"];
const DEFAULT_PRODUCT_KNOWLEDGE = "① 에센셜 리제너레이터 핸드크림 (50ml, 실내용)\n   - “손 노화를 막고 손의 시간을 되돌리는” NMN 기반 손티에이징(손 안티에이징) 핸드크림. 단순 보습이 아니라 손 역노화에 초점. 올리브영 1,400개 매장·2025 바디케어 1위·올영 어워즈. (“국내 유일” 같은 표현은 쓰지 않는다.)\n   - 핵심 성분: NMN(순도 99.9%, 손 세포 역노화·손결 회복) + 나이아신아마이드(손등 칙칙함·기미·톤 불균형 케어 → 밝고 정돈된 손) + 아데노신(손등 탄력·주름 개선) + 시어버터(보습). 주름개선·미백 2중 기능성.\n   - 임상(4주 사용): 손 피부 탄력 45.5% 개선 — 손 거칠기 17.9%↓, 보습 17.3%↑, 주름 11.3%↓, 탄력 13.3%↑. “임상으로 증명된 손티에이징 핸드크림”으로 신뢰감 있게 설명할 수 있다.\n   - 실내(잦은 세정·건조·마찰로 수분 장벽이 무너지는 환경)에서 손 보습·컨디션 유지에 집중. 무향에 가깝고 촉촉하지만 꾸덕하지 않아 실내·사무실에서 수시로 덧발라 쓰기 좋다.\n\n② 어드밴스드 리제너레이터 핸드크림 (30ml, SPF20/PA++, 실외용)\n   - 에센셜의 손 역노화(NMN·나이아신아마이드·아데노신) 성분은 그대로 두고 “자외선 차단”을 더한 야외 전용 손티에이징 핸드크림. 자외선 차단 외 성분은 에센셜과 동일하다.\n   - 자외선 차단을 위해 SPF20/PA++가 들어 있다(성분명은 언급하지 말고 “SPF20으로 자외선 차단” 정도로만 설명한다).\n   - 운전·야외활동·이동 중 손등이 자외선에 그대로 노출되는 환경(자외선 → 손 노화 가속)을 겨냥. 외출 전·외출 중 수시로.\n\n②-세트 핸드크림 인앤아웃 세트 (에센셜 실내용 + 어드밴스드 야외용, 핸드크림 2종 구성)\n   - ★리뷰 상품이 “핸드크림 인앤아웃 세트”이면 어드밴스드만 이야기하지 말고, 에센셜(실내용)과 어드밴스드(야외용) 두 제품의 특징을 균형 있게 섞어 설명한다.\n   - 개발 의도: 손은 하루 동안 실내(세정·건조·마찰로 수분 장벽 무너짐)와 야외(자외선으로 손 노화 가속)라는 전혀 다른 두 환경에 노출된다. 그래서 장소에 따라 실내용 에센셜(보습·손 컨디션 유지)과 야외용 어드밴스드(SPF20 자외선 차단)로 나눠 쓰도록 만든 세트다. 이 “실내·야외 구분” 흐름을 자연스럽게 녹여 손 안팎을 함께 케어한다는 톤으로 답한다.\n   - (주의: 여기서 “인앤아웃”은 핸드크림 2종 세트를 뜻하며, 선크림 EGA UV IN&OUT Protector와 혼동하지 않는다.)\n\n③ NMN 스킨부스터 (NMN Daily Routine for Women, 먹는 이너뷰티)\n   - 역노화 NMN 프리미엄 건강식품. 청담·신사 피부과에서도 제공. NAD+ 부스팅으로 컨디션·피부 활력 개선.\n   - 핵심 성분: NMN 250mg(99.9%) + Dermial 60mg(히알루론산·콜라겐·엘라스틴). 1포 2g, 30스틱, 제로 칼로리, 복숭아맛이라 물 없이 간편.\n   - 복용법: 매일 일정한 시간에 규칙적으로, 아침 식전 공복 섭취 권장. 꾸준함이 핵심.\n\n④ NMN 지우개 마스크팩 (White Repair Face & Neck Kit, 겔 마스크팩)\n   - 컨셉: “자외선 쐰 날 바로 붙이는 애프터 선(After Sun) 마스크팩”.\n   - 핵심 성분: NMN 순수 1,000ppm(99.9%), CefiraProtect CLR™(세피라 프로텍트), 나이아신아마이드, 아데노신.\n   - ★필수: 자외선·UV·광노화 관련 효과를 설명할 때는 반드시 “세피라 프로텍트(CefiraProtect CLR™)” 성분을 근거로 함께 언급한다. 세피라 프로텍트는 자외선을 비롯한 환경 스트레스로 손상된 피부를 보호하고 피부 본연의 방어력을 지지하는 기능성 성분으로, EGA 마스크팩의 UV 리페어·애프터선 케어의 핵심이다. (NMN은 세포 단위 회복, 세피라 프로텍트는 자외선 방어·보호를 담당하는 두 축으로 설명하면 좋다.)\n   - 특징: 달아오른 피부 즉각 진정·쿨링 / UV 리페어·화이트닝(기미·잡티 완화, 맑은 톤) / 얼굴+목(Face&Neck) 동시 케어.\n   - 사용법(반드시 이 내용으로 안내): 스킨케어 후 얼굴과 목에 밀착시켜, 시트가 완전히 투명해질 때까지 붙여 둔다. 시트가 투명해지는 것은 유효 성분이 피부에 충분히 흡수되었다는 신호이며, 피부 타입에 따라 평균 40분~1시간 정도 걸린다. 때에 따라 수면팩으로도 활용할 수 있다.\n\n⑤ NMN UV Healer (5% 앰플, UV 케어 앰플)\n   - NMN 5% 고함량 앰플(나이아신아마이드·아데노신·글리세린). 5ml×4바이알.\n   - 자외선 케어와 집중 회복이 필요한 부위에 앰플로 사용.\n\n⑥ EGA UV IN&OUT Protector (얼굴 데일리 선크림)\n   - 컨셉: 이름 그대로 “자외선은 막고(OUT), 세포 안에서는 NMN으로 회복(IN)” 선크림. 판매가 31,500원.\n   - 무기자차와 유기자차를 함께 쓴 혼합자차. 백탁 없이 로션처럼 발리고, 끈적임·답답함이 없어 화장이 잘 먹는다는 반응.\n   - ★중요(꼭 활용): 바를 때 느껴지는 살짝 화하고 시원한 느낌(쿨링감·플럼핑감·따끔한 청량감)은 피부 자극이 아니라, 제품에 담긴 ‘디퍼프(de-puff) 에이전트’가 작용하는 신호다. 이 성분이 피부 표면의 붓기(부기)를 가라앉히고 순환을 도와 순간적으로 화하고 시원한 느낌이 드는 것으로, 붓기가 빠지면서 피부결이 정돈되고 얼굴 라인이 매끈해지는 정상적인 과정이다. 리뷰에서 “화하다·시원하다·쿨링·따끔·플럼핑” 등을 언급하면, 이 원리를 과학적으로 그리고 안심되게 설명해 만족감을 높인다. (부작용이 아님을 분명히 한다.)\n   - 올바른 사용법(반드시 이 내용으로 안내): 아침 기초케어(스킨케어)의 가장 마지막 단계에, 외출 약 20분 전에 충분한 양을 덜어 자외선 노출 부위에 꼼꼼히 펴 발라 준다. 야외활동이 길거나 자외선이 강한 날에는 2~3시간마다 덧발라 주는 것이 가장 효과적이다. (“클렌징 직후”가 아니라 외출 전 마지막 단계라는 점을 정확히 전달한다.)\n   - 매일 데일리로 사용하기 좋고, 여름·자외선 강한 시즌에 특히 추천.";
let PROMPT_CONFIG = null;
function loadPromptConfig() {
  try {
    const raw = JSON.parse(fs.readFileSync(PROMPT_CONFIG_PATH, "utf8"));
    if (raw && raw.products && Object.keys(raw.products).length) { PROMPT_CONFIG = raw; return raw; }
  } catch (e) {}
  return PROMPT_CONFIG;
}
loadPromptConfig();
function activeProductKnowledge() {
  if (PROMPT_CONFIG && PROMPT_CONFIG.products) {
    const order = (PROMPT_CONFIG.order && PROMPT_CONFIG.order.length) ? PROMPT_CONFIG.order : DEFAULT_PRODUCT_ORDER;
    const blocks = order.map((k) => PROMPT_CONFIG.products[k]).filter(Boolean);
    if (blocks.length) return blocks.join("\n\n");
  }
  return DEFAULT_PRODUCT_KNOWLEDGE;
}
function commitPromptConfig(jsonStr) {
  return new Promise((resolve) => {
    const token = process.env.GH_TOKEN || process.env.GITHUB_TOKEN;
    const repo = process.env.CONFIG_REPO || "softdubu-spec/EGA_Review_Insight_Archive";
    if (!token) return resolve({ committed: false, reason: "no_token" });
    const H = { "User-Agent": "ega-reply", "Authorization": "Bearer " + token, "Accept": "application/vnd.github+json" };
    const g = https.request({ hostname: "api.github.com", path: "/repos/" + repo + "/contents/prompt_config.json", method: "GET", headers: H }, (r) => {
      let d = ""; r.on("data", (c) => (d += c)); r.on("end", () => {
        let sha = null; try { sha = JSON.parse(d).sha; } catch (e) {}
        const body = JSON.stringify({ message: "prompt: 웹에서 제품 프롬프트 수정", content: Buffer.from(jsonStr, "utf8").toString("base64"), sha: sha, branch: "main" });
        const p = https.request({ hostname: "api.github.com", path: "/repos/" + repo + "/contents/prompt_config.json", method: "PUT", headers: Object.assign({}, H, { "Content-Type": "application/json", "Content-Length": Buffer.byteLength(body) }) }, (rr) => {
          let dd = ""; rr.on("data", (c) => (dd += c)); rr.on("end", () => resolve({ committed: rr.statusCode >= 200 && rr.statusCode < 300, status: rr.statusCode }));
        });
        p.on("error", () => resolve({ committed: false, reason: "put_error" })); p.write(body); p.end();
      });
    });
    g.on("error", () => resolve({ committed: false, reason: "get_error" })); g.end();
  });
}
...
- 자외선·UV·광노화 관련 효능을 언급할 때는 반드시 담당 성분을 함께 짚어 전문성을 준다.
  ...
  · 어드밴스드 핸드크림 → 성분명(에틸헥실트리아존 등)은 쓰지 말고 “SPF20이 들어 있어 자외선 차단” 정도로만 설명한다.
  · 선크림(UV IN&OUT Protector) → 무기자차+유기자차 혼합자차 + NMN(속 회복).
- 성분 이름을 반드시 정확히 쓰고, 자외선 효과를 성분 근거 없이 두루뭉술하게 말하지 않는다.
- ★단, 각 성분은 그 성분이 실제로 든 “구매 제품”을 설명할 때만 쓴다. 예: 세피라 프로텍트(CefiraProtect CLR™)는 마스크팩을 구매한 리뷰에서만 언급하고, NMN 스킨부스터·핸드크림 리뷰에는 마스크팩 성분(세피라 프로텍트)을 끌어오지 않는다. (예외: 선크림 리뷰에서 애프터선 케어로 마스크팩을 크로스셀할 때는, 마스크팩의 자외선 데미지 회복 근거로 세피라 프로텍트를 짧게 언급해도 된다.)

[제품 지식]에 근거해 구체적으로 설명. 성분은 1~2개만 자연스럽게 언급하고 나열식은 피한다.
- 3문단: 반드시 고객이 “구매한 제품”의 사용법/복용법만 안내한다. 고객이 사지 않은 다른 제품의 사용법·성분은 이 문단에서 설명하지 않는다. 그 뒤에 계절·니즈에 맞는 EGA 연관 제품 1개만 “새로 소개·권유”하는 톤으로 짧게 추천한다. 추천 제품의 상세 사용법·성분은 나열하지 않는다.
...
- ★개별화(부정·미온적 리뷰일수록 필수): 정해진 문구를 반복하지 말고 이 고객만의 상황(복용 기간·구매 통수·직접 언급한 고민)을 그대로 문장에 녹여 1:1로 답한다. 그리고 체내 기본 NAD+ 수치와 대사 속도, 생활습관·컨디션은 사람마다 다르기 때문에, 같은 기간을 드셔도 체감되는 시점과 정도가 개인마다 다르게 나타날 수 있다는 점을 부드럽게 설명한다. 이는 제품이 안 맞는 것이 아니라 자연스러운 개인차이며, 그래서 본인의 리듬을 찾아가는 꾸준한 복용이 중요하다는 점을 재촉이 아닌 신뢰감 있는 톤으로 전달한다.
- 마무리는 재촉이 아니라 신뢰감 있게: 최소 2~3주 이상 꾸준히 사용하며 피부·컨디션 변화를 지켜봐 달라고 제안하고, 그 과정에 EGA가 함께하겠다는 뉘앙스로 자연스럽게 재사용·재구매를 유도한다.

[금지]
- 마케팅 신조어나 임의 별칭을 지어내지 않는다. (예: “투인원”, “2-in-1”, “올인원”, “이중 보호”, “종결템”, “꿀템” 등 금지) 제품은 위 [제품 지식]에 적힌 단어·표현 그대로만 설명한다.
- 고객이 구매하지 않은 제품의 사용법·성분·효능을 자세히 설명하지 않는다. 크로스셀(연관 제품)은 “새로 나온 ~를 한 번 경험해보시길 권해드린다” 수준의 짧은 소개·권유로만 한다.
...
app.get("/api/prompt-config", (req, res) => {
  res.json({ ok: true, config: PROMPT_CONFIG || { products: {}, labels: {}, order: DEFAULT_PRODUCT_ORDER }, usingDefault: !PROMPT_CONFIG });
});
app.post("/api/prompt-config", async (req, res) => {
  try {
    const incoming = (req.body && req.body.config) ? req.body.config : req.body;
    if (!incoming || !incoming.products || typeof incoming.products !== "object" || !Object.keys(incoming.products).length) {
      return res.status(400).json({ ok: false, message: "products가 비어 있습니다." });
    }
    const merged = Object.assign({}, PROMPT_CONFIG || {}, incoming);
    merged.order = (merged.order && merged.order.length) ? merged.order : DEFAULT_PRODUCT_ORDER;
    merged.updatedAt = new Date().toISOString();
    PROMPT_CONFIG = merged;
    const jsonStr = JSON.stringify(merged, null, 2);
    try { fs.writeFileSync(PROMPT_CONFIG_PATH, jsonStr); } catch (e) {}
    const commit = await commitPromptConfig(jsonStr);
    res.json({ ok: true, applied: true, committed: commit.committed, commit: commit });
  } catch (error) {
