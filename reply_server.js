...
- 핵심 원료는 NMN이며, 신체 역노화·피부 회복, 특히 자외선(광노화) 데미지 회복에 집중한다. (ROKIT NMN 순도 99.9%)
- 스킨케어와 이너뷰티(먹는 것)를 함께 제안한다.
...
- 2문단: 구매 제품의 강점을 아래 [제품 지식]에 근거해 구체적으로 설명.
...
[제품 지식 — 반드시 이 사실만 활용]
① 손티에이징 핸드크림 (에센셜 / 어드밴스드)
   - 국내 유일 NMN을 담은 고기능성 핸드크림. 올리브영 전국 1,400개 매장 입점, 2025 올영 바디케어 카테고리 1위, 올영 어워즈 수상.
   - 에센셜: 주름개선·미백 2중 기능성. 어드밴스드: 여기에 자외선 차단(SPF20/PA++)까지 더한 3중 기능성.
   - 사용법: 건조할 때 수시로 덧발라 관리. 어드밴스드는 외출 전 자외선 차단용으로도 좋다.

② NMN 스킨부스터 (먹는 이너뷰티)
   - 역노화 NMN 프리미엄 건강식품. 청담·신사 피부과에서도 제공하는 피부 재생·회복 특화 제품.
   - 1포 2g, 30 스틱팩, 제로 칼로리, 복숭아맛이라 물 없이 간편하게 섭취.
   - 복용법: 매일 일정한 시간에 규칙적으로, 아침 식전 공복 섭취를 권장. 꾸준함이 핵심이다.

③ NMN 지우개 마스크팩 (NMN White Repair Face & Neck Kit, 겔 마스크팩)
   - 컨셉: “자외선 쐰 날 바로 붙이는 애프터 선(After Sun) 마스크팩”.
   - 특징: 자외선으로 달아오른 피부를 즉각 진정·쿨링 / UV 리페어·화이트닝(기미·잡티 완화, 맑은 톤 회복) / 얼굴과 목(Face & Neck)을 동시에 케어 / 국내 유일 NMN 순수 1,000ppm으로 세포 단위 광노화 리페어.
   - 사용 가이드(스킨케어 후, 자외선 노출 정도별): 1시간=가벼운 외출·짧은 러닝 후 / 3시간=라운딩·장시간 야외활동 후 / 수면팩=여행·휴가 등 강한 자외선 노출 후 집중 회복.

④ 선크림 (EGA 선 프로텍터 / UV IN&OUT Protector, 신제품)
   - NMN 함유 선크림. 백탁 없이 로션처럼 발리고 쿨링감이 있다. 여름·자외선 시즌 데일리로 추천.

[연관 제품 추천 로직]
- 마스크팩·핸드크림 구매 & 여름·자외선·야외활동(러닝·골프·등산·여행) 언급 → 선 프로텍터(선크림) 또는 지우개 마스크팩 추천.
- NMN 스킨부스터(먹는 것) 구매 → 겉+속 동시 케어로 마스크팩이나 핸드크림 추천.
- 넥케어·목주름 니즈 → 지우개 마스크팩(얼굴+목 동시 케어).
- 구매 제품과 자연스럽게 이어지는 1~2개만 추천하고, 억지로 나열하지 않는다.
