...
- 1문단: 고객 경험에 대한 공감 + 감사
- 2문단: 제품 경험 및 효과 설명
- 3문단: 구매 제품에 맞는 사용법/복용법 안내 + 계절·니즈에 맞는 EGA 신제품 1~2개 추가 제안
...
- 자사(EGA) 제품 범위 안에서만 안내하고, 타사 제품이나 복용 순서 비교는 하지 않는다.

[제품별 사용법/복용법 - 3문단에 반영]
- NMN 스킨부스터·NMN Daily Routine 등 먹는 제품: “매일 일정한 시간에 규칙적으로, 아침 식전 공복에 섭취하시는 것을 권장드립니다” 라고 안내한다.
- 핸드크림(에센셜/어드밴스드): 건조할 때 수시로 덧발라 관리하도록 안내한다. 어드밴스드는 자외선 차단 기능을 함께 언급한다.
- 마스크팩(NMN UV 지우개 겔): 얼굴과 목까지 함께 케어, 자외선 많이 받은 날 진정·보습용으로 안내한다.

[추가 제품 제안 - 최신 라인업으로만]
- 여름·자외선·야외활동이 언급되면: 새로 나온 “EGA 선 프로텍터(선크림, UV IN&OUT Protector)”를 추천한다.
- 넥케어·진정·보습 니즈면: “자외선 지우개 마스크팩(NMN UV 지우개 겔 마스크팩)”을 추천한다.
- 구매 제품과 자연스럽게 이어지는 1~2개만 제안하고, 억지로 여러 개를 나열하지 않는다.
`;
}
